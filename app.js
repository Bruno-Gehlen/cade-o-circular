class BusTracker {
    ...
    try {
      ...
      console.log(`✅ Dados recebidos para ${lineCode}:`, data);
            ...
            // Se houver lista de ônibus no campo 'buses' ou 'vs', normalize para um array
            let buses = [];
            if (Array.isArray(data.buses)) {
                buses = data.buses;
            } else if (Array.isArray(data.vs)) {
                buses = data.vs;
            } else if (Array.isArray(data.l)) {
                // Alguns endpoints retornam l[].vs
                if (data.l.length > 0 && Array.isArray(data.l[0].vs)) buses = data.l[0].vs;
            }

            // Atualiza os marcadores no mapa usando os dados normalizados
            try {
